...
// Using the IIFE to prevent being in  globalname space.
// angular will be the only module in global namespace,
(function(){
// Module aboutController and ngAnimate as a dependency.
var aboutControllers = angular.module('aboutControllers', ['ngAnimate']);

/* /////////////////////*/
  /* ListController*/
/* /////////////////////*/
var ListController = function($scope, $http, $anchorScroll, $location ) {

  // access the mongolab database using get request
  var myApi = $http.get('https://api.mlab.com/api/1/databases/myskills/collections/aboutme?apiKey=8pSbP2J_sw_ot9D5c1vbgDcl2wuSG2FH');
      
  // Error handler
  var error= function(){         
      $scope.error= "Database is down";
  };
...
var DetailsController = function($scope, $http, $routeParams) {
  ...
  myApi.success(function(data) {
    ...
    if ($routeParams.itemId > 0) {
      ...
      $scope.prevItem = $scope.aboutme.length-1;
    }

    if ($routeParams.itemId < $scope.aboutme.length-1) {
...
// Registration of artistController so the controller will take responsibility for the specific module.
// ListConteoller as a sub controller
// Passing the $scope and $http so that prevent them from minification during produvtion.
aboutControllers.controller('ListController', ['$scope', '$http', '$location', '$anchorScroll', ListController]);
aboutControllers.controller('DetailsController', ['$scope', '$http','$routeParams', DetailsController]);
}()); //IIFE
